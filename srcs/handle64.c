...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/10/17 13:08:36 by djanusz           #+#    #+#             */
...
char handle_64_type(Elf64_Shdr *shdr, Elf64_Sym sym)
{
	...
	else if (ELF64_ST_BIND(sym.st_info) == STB_WEAK)
	{
		...
		c = '?';
	if (ELF64_ST_BIND(sym.st_info) == STB_LOCAL && ('A' <= c && c <= 'Z') && c != 'N')
		c += 32;
	return c;
}
...
void handle_64_symbols(char *ptr, Elf64_Ehdr *header, Elf64_Shdr *sections, Elf64_Shdr *symTab)
{
	...
	for (int i = 1; i < symTabSize; i++)
	{
		...
		tab[i - 1].type = handle_64_type(sections, symbol[i]);
	...
	print_symbols(tab, symTabSize, 16);
