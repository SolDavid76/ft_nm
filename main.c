...
/*   Updated: 2024/09/24 15:23:55 by djanusz          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_symbol *handle_64_symbols_sort(t_symbol *tab, int tabSize)
{
	for (int i = 0; i < tabSize - 1; i++)
	{
		for (int j = i + 1; j < tabSize - 1; j++)
		{
			...
			if (ft_strcmp(tab[i].name, tab[j].name) == 0 && tab[i].adress > tab[j].adress)
				ft_swap(&tab[i], &tab[j]);
...
void print_64_symbols(t_symbol *tab, int tabSize)
{
	for (int i = 0; i < tabSize - 1; i++)
	{
		if (tab[i].type != 'U' && tab[i].type != 'w')
		{
			for (int j = hexalen(tab[i].adress); j < 16; j++)
				write(1, "0", 1);
			ft_putnbr_base(tab[i].adress, "0123456789abcdef");
		}
		else
			write(1, "                ", 16);
		write(1, " ", 1);
		write(1, &tab[i].type, 1);
		write(1, " ", 1);
		write(1, tab[i].name, ft_strlen(tab[i].name));
		write(1, "\n", 1);
	}
}
...
// 00001 = -p (no sort)
// 00010 = -r (reverse sort)
// 00100 = -a (all)
// 01000 = -g (extern only)
// 10000 = -u (undefined only)
